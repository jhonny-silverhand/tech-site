...
/**
 * Reliance Digital Product Provider (via Apify Actor API)
 * 
 * NOTE: Reliance Digital doesn't have a public API. Options for production:
 * 1. Reliance Digital Affiliate Program (via Cuelinks, EarnKaro, etc.)
 * 2. Third-party data aggregators like:
 *    - Apify Reliance Digital Product Scraper (sian.agency~reliance-digital-product-scraper)
 *    - Bright Data Reliance Digital Scraper
 *    - Actowiz Solutions Reliance Digital Data Scraping
 * 
 * For development/testing, we use mock data.
 * For production, configure RELIANCE_DIGITAL_APIFY_TOKEN to use Apify Actor API.
 * 
 * Apify Actor: https://apify.com/sian.agency/reliance-digital-product-scraper
 * Affiliate Program: https://www.cuelinks.com/campaigns/reliance-digital-affiliate-program
 */

interface RelianceDigitalProduct {
  productTitle?: string;
  brand?: string;
  model_number?: string;
  item_code?: string;
...
interface RelianceDigitalSearchResponse {
  data?: RelianceDigitalProduct[];
  totalResults?: number;
  error?: string;
}

interface RelianceDigitalProductDetailResponse {
  product?: RelianceDigitalProduct;
  error?: string;
}
...
export class RelianceDigitalProvider {
  private config = {
    name: 'Reliance Digital',
    baseUrl: 'https://api.apify.com/v2',
    actorId: 'sian.agency~reliance-digital-product-scraper',
    apiToken: process.env.RELIANCE_DIGITAL_APIFY_TOKEN,
    rateLimit: {
      requestsPerMinute: 10,
      requestsPerHour: 100,
    },
    timeout: 30000,
    enabled: !!process.env.RELIANCE_DIGITAL_APIFY_TOKEN,
    priority: 4,
  };
  ...
  async searchProducts(params: SearchProductsParams): Promise<SearchProductsResult> {
    ...
    try {
      ...
      const response = await this.makeApifyRequest<RelianceDigitalProduct[]>(url, cleanBody);
      
      const products: ProviderProduct[] = (response || []).map((item, index) => 
        this.transformRelianceDigitalProduct(item, index)
      ...
      console.error('[RelianceDigitalProvider] searchProducts error:', error);
      return this.getMockSearchResults();
    }
  }
  ...
  async getProductBySlug(slug: string): Promise<ProductBySlugResult | null> {
    ...
    try {
      ...
      const response = await this.makeApifyRequest<RelianceDigitalProduct[]>(`${this.config.baseUrl}/acts/${this.config.actorId}/run-sync-get-dataset-items`, body);
      
      if (response.length === 0) return null;

      return this.transformRelianceDigitalProduct(response[0], 0)!;
    } catch (error) {
      console.error('[RelianceDigitalProvider] getProductBySlug error:', error);
      return this.getMockProductBySlug();
    }
  }
  ...
  async getProductsByCategory(categorySlug: string, limit = 50): Promise<ProviderProduct[]> {
    ...
    try {
      ...
      console.error('[RelianceDigitalProvider] getProductsByCategory error:', error);
      return this.getMockProductsByCategory(categorySlug, limit);
    }
  }
  ...
  private transformRelianceDigitalProduct(item: RelianceDigitalProduct, index: number): ProviderProduct | null {
    ...
    if (item.original_price && item.original_price !== item.price) {
      retailers.push({
        retailerId: 'reliancedigital-mrp',
    ...
    return {
      externalId: `rd-${item.item_code || item.ean || `product-${index}`}`,
      name: item.productTitle,
      slug: (item.item_code || item.ean || `rd-product-${index}`).toLowerCase(),
  ...
  private extractSpecs(item: RelianceDigitalProduct): Array<{ key: string; value: string; unit?: string; displayOrder: number }> {
  ...
  private inferCategory(item: RelianceDigitalProduct): string {
    ...
    if (allText.includes('earbud') || allText.includes('earbuds')) return 'earbuds';
    return 'electronics';
  }
      ...
      products: [
        {
          ...
          retailers: [
            { retailerId: 'reliancedigital', price: 96900, currency: 'INR', availability: 'in_stock', url: 'https://www.reliancedigital.in/apple-macbook-air-m2', affiliateUrl: undefined, lastChecked: new Date().toISOString() },
        ...
        },
        {
          ...
          retailers: [
            { retailerId: 'reliancedigital', price: 113900, currency: 'INR', availability: 'in_stock', url: 'https://www.reliancedigital.in/dell-xps-13', affiliateUrl: undefined, lastChecked: new Date().toISOString() },
