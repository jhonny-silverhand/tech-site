...
export async function getFeaturedProductsForHero(): Promise<FeaturedProduct[]> {
  ...
  if (isSupabaseConfigured()) {
    // Try to get real products from Supabase — parallelize all 3 categories
    try {
      const results = await Promise.all(
        categories.map(async (cat) => {
          ...
          if (products.length > 0) {
            ...
            if (withRetailers) return { category: cat, product: withRetailers };
          }
          return null;
        })
      );
      const featured = results.filter(Boolean) as FeaturedProduct[];
...
export async function getRecommendations(input: RecommendationInput): Promise<ProductRecommendation[]> {
  try {
    ...
    // Limit to 10 to avoid N+1 query explosion (each getProductWithRetailers does 3 queries)
